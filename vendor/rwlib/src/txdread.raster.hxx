...
{
    typedef StaticPluginClassFactory <Raster, RwDynMemAllocator> rasterFactory_t;
...
struct nativeTextureStreamPlugin : public serializationProvider
{
    ...
    inline void operator = ( const nativeTextureStreamPlugin& right )
    {
        throw RwException( "cannot clone RenderWare native texture stream environment" );
    }
    ...
    {
        void OnWarningMessage( rwStaticString <char>&& theMessage ) override
        {
            this->message_list.AddToBack( theMessage );
        }

        typedef rwStaticVector <rwStaticString <char>> messages_t;
                ...
                else
                {
                            ...
                            if ( nativeData )
                            {
                                try
                                {
                                    ...
                                    catch( RwException& theError )
                                    {
                                        ...
                                        if ( theError.message.GetLength() != 0 )
                                        {
                                            engineInterface->PushWarning( rwStaticString <char> ( theError.message ) );
                                        }
                                    }
                                }
                                catch( ... )
                    ...
                    if ( !hasSuccessfullyDeserialized )
                    {
                        ...
                        for ( size_t n = 0; n < maybeCount; n++ )
                        {
                            ...
                            rwString <char> typeWarnBuf( eir::constr_with_alloc::DEFAULT, intf );
    ...
    {
        AINLINE nativeTextureCustomTypeInterface( Interface *engineInterface, texNativeTypeProvider *texTypeProvider, size_t actualObjSize )
        {
            ...
            if ( platformTexType != nullptr )
            {
                ...
                try
                {
                    newType = engineInterface->typeSystem.RegisterCommonTypeInterface <nativeTextureCustomTypeInterface> (
                        nativeName, platformTexType, engineInterface, typeProvider, memSize
                    );
                }
                catch( ... )
                {

...
struct d3d12DriverInterface : public nativeDriverImplementation
{
    ...
    NativeExecutive::execThreadStructPluginRegister <threadLocalEnv> threadLocalEnvPlugin;
    ...
    typedef StaticPluginClassFactory <d3d12NativeDriver, RwDynMemAllocator> d3d12DriverFactory_t;
    ...
    void FenceSingleWaitForSignal( void *objMem, uint64 waitValue, uint32 timeout ) override
    {
        ...
        if ( threadLocalEnv *threadEnv = threadLocalEnvPlugin.GetPluginStructCurrent() )
        {
    ...
    inline void Initialize( EngineInterface *engineInterface )
    {
        ...
        if ( NativeExecutive::CExecutiveManager *execMan = GetNativeExecutive( engineInterface ) )
        {
            ...
            execMan->PurgeActiveThreads();

            threadLocalEnvPlugin.RegisterPlugin( execMan );
        }
    ...
    inline void Shutdown( EngineInterface *engineInterface )
    {
        ...
        // Terminate the thread local environment.
        if ( NativeExecutive::CExecutiveManager *execMan = GetNativeExecutive( engineInterface ) )
        {
            this->threadLocalEnvPlugin.UnregisterPlugin();
        }
    }
};

// Common definitions such as strings or vectors that are used by magic-rw.
...
#define RW_IMPL_HEAP_REDIR_ALLOC_INTERFACE_PUSH( hostStructType, redirAllocType, redirNode, interfaceFieldName ) \
    ...
    { \
        ...
        return hostStruct->interfaceFieldName->MemAllocate( memSize, alignment ); \
    } \
    IMPL_HEAP_REDIR_METH_RESIZE_RETURN hostStructType::redirAllocType::Resize IMPL_HEAP_REDIR_METH_RESIZE_ARGS \
    { \
        hostStructType *hostStruct = LIST_GETITEM( hostStructType, refMem, redirNode ); \

...
struct RwException
{
    inline RwException( const rwStaticString <char>& msg )
    {
        this->message = msg;
    }

    inline RwException( rwStaticString <char>&& msg )
    {
        this->message = std::move( msg );
    }

    rwStaticString <char> message;
};
...
{
    virtual void OnWarning( rwStaticString <char>&& message ) = 0;
};
...
public:
    ...
    bool                MemResize               ( void *ptr, size_t memSize );
    void                MemFree                 ( void *ptr );

    void*               PixelAllocate           ( size_t memSize, size_t alignment = sizeof(uint32) );
    bool                PixelResize             ( void *ptr, size_t memSize );
    ...
    // Warning system.
    void                PushWarning             ( rwStaticString <char>&& message );
    void                PushObjWarningVerb      ( const RwObject *theObj, const rwStaticString <char>& verbMsg );
...
{
    return this->engineInterface->MemAllocate( memSize, alignment );
}
IMPL_HEAP_REDIR_METH_RESIZE_RETURN RwDynMemAllocator::Resize IMPL_HEAP_REDIR_METH_RESIZE_ARGS
{

...
public:
    ...
    AINLINE void ScanSharedSlices( sectorSlice_t sliceShared, const callbackType& cb, bool includeNotPresent = false, bool availableSliceWhole = false )
    {
        ...
        if ( avlBeginNode == nullptr )
            return;

        lowerBound <numberType> curStartBound = sliceShared.GetSliceStartBound();

        typename avlSliceTree_t::diff_node_iterator iter( avlBeginNode );

        sliceNode *curNode = AVL_GETITEM( sliceNode, avlBeginNode, node );

        sectorSlice_t curSlice = curNode->GetNodeSlice();
        ...
        while ( true )
        {
            if ( includeNotPresent )
            {
                ...
                {
                    const sectorSlice_t unavailSlice = sectorSlice_t::fromBounds( curStartBound, GetTightBound( curSlice.GetSliceStartBound() ) );

                    cb( unavailSlice, nullptr );
                }

                // Update the current start point.
                if ( curStartBound <= curSlice.GetSliceEndBound() )
                {
                    curStartBound = GetTightBound( curSlice.GetSliceEndBound() );
                }
            }

            // Now report the available slice.
            {
                if ( availableSliceWhole )
                {
                    cb( (const sectorSlice_t&)curSlice, (const metaDataType*)&curNode->metaData );
                }
                else
                {
                    sectorSlice_t objSharedSlice;
...
#ifdef _DEBUG
                    assert( gotShared == true );
#endif //_DEBUG

                    cb( (const sectorSlice_t&)objSharedSlice, (const metaDataType*)&curNode->metaData );
                }
            }
            ...
            // Fetch the next node pointer.
            curNode = AVL_GETITEM( sliceNode, iter.Resolve(), node );

            curSlice = curNode->GetNodeSlice();

            // Is that node still in the region?
            eCompResult cmpRes = _CompareSlices( curSlice, sliceShared, false );

            if ( cmpRes != eCompResult::EQUAL )
            {
                break;
            }
        }
        ...
        if ( includeNotPresent )
        {
            ...
            {
                const sectorSlice_t unavailSlice = sectorSlice_t::fromBounds( curStartBound, sliceShared.GetSliceEndBound() );
...
private:
    static AINLINE eCompResult _CompareSlices( const sectorSlice_t& leftSlice, const sectorSlice_t& rightSlice, bool doCountInNeighbors )
    {
    ...
    static AINLINE eCompResult _CompareNodeWithSlice( const AVLNode *left, const sectorSlice_t& rightSlice, bool doCountInNeighbors )
    {
        ...
        return _CompareSlices( leftSlice, rightSlice, doCountInNeighbors );
    }

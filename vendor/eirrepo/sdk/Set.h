...
// Default comparator for objects inside the Set.
struct SetDefaultComparator
{
    template <typename firstValueType, typename secondValueType>
    static AINLINE bool is_less_than( const firstValueType& left, const secondValueType& right )
    {
        // This is important because not all value types come with a usable "operator <" overload,

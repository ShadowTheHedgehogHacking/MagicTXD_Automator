...
private:
    ...
    struct VMemArray
    {
        ...
        inline VMemArray( NativeVirtualMemoryAccessor& vmemAccess, size_t arenaSize )
        {
            ...
            assert( reqOffTo_aligned <= arenaSize );
        ...
        inline void Resize( NativeVirtualMemoryArrayAllocator *manager, size_t newSize )
        {
            ...
            assert( newSize <= this->blockAllocSize );
            assert( ( newSize % manager->GetVirtualMemoryAccess().GetPlatformPageSize() ) == 0 );

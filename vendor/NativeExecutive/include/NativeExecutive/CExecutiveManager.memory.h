...
{
    inline NatExecStandardObjectAllocator( CExecutiveManager *natExec )
    {
    ...
    inline void* Allocate( void *refPtr, size_t memSize, size_t alignment )
    {
        return manager->MemAlloc( memSize, alignment );
    }

    inline bool Resize( void *refPtr, void *memPtr, size_t reqSize )
    {
        return manager->MemResize( memPtr, reqSize );
    }

    inline void Free( void *refPtr, void *memPtr )
    {

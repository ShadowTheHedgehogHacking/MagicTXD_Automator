...
struct natEventInst
{
    inline natEventInst( CExecutiveManager *manager ) : manager( manager )
    {
...
{
    inline CUnfairMutexContext( CUnfairMutex *mtx )
    {
        if ( mtx )
        {
            mtx->lock();
    ...
    inline CUnfairMutexContext( CUnfairMutexContext&& ) = delete;
    ...
    inline CUnfairMutexContext& operator = ( CUnfairMutexContext&& ) = delete;
...
AINLINE CExecThread* CreateThreadL( CExecutiveManager *nativeMan, callbackType&& cb, size_t stackSize = 0 )
{
    struct helpers
    {
        static void lambda_runtime( CExecThread *thread, void *ud )
        {
    ...
    CExecThread *theThread = nativeMan->CreateThread( helpers::lambda_runtime, storedLambda, stackSize );

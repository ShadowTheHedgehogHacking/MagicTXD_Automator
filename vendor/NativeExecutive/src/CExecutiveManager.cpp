...
// Factory API.
optional_struct_space <executiveManagerFactory_t> executiveManagerFactory;

// Memory allocator for static data, because it actually is required.
static optional_struct_space <NativeHeapAllocator> _staticHeapAlloc;

void* nativeExecutiveAllocator::Allocate( void *refPtr, size_t memSize, size_t alignment )
{
    return _staticHeapAlloc.get().Allocate( memSize, alignment );
}

bool nativeExecutiveAllocator::Resize( void *refPtr, void *memPtr, size_t reqSize )
{
    return _staticHeapAlloc.get().SetAllocationSize( memPtr, reqSize );
}

void nativeExecutiveAllocator::Free( void *refPtr, void *memPtr )
{
...
void dereference_library( void )
{
    if ( --_initRefCount == 0 )
    {
...
#ifdef _DEBUG
        {
            ...
            NativeHeapAllocator& heapAlloc = _staticHeapAlloc.get();

            NativeHeapAllocator::heapStats stats = heapAlloc.GetStatistics();
...
struct basicFiberObjectConstructor
{
    ...
    AINLINE basicFiberObjectConstructor( CExecutiveManagerNative *manager )
    {
        this->manager = manager;
    }

    AINLINE CFiberImpl* Construct( void *mem ) const
    {
        return new (mem) CFiberImpl( this->manager, nullptr, nullptr );
    }
};
...
CFiber* CExecutiveManager::CreateFiber( CFiber::fiberexec_t proc, void *userdata, size_t stackSize )
{
    ...
    CFiberImpl *fiber = nullptr;
    {
        basicFiberObjectConstructor constructor( nativeMan );

        fiber = fiberEnv->fiberFact.ConstructTemplate( objMemAlloc, constructor );
    }
...
void CExecutiveManager::TerminateFiber( CFiber *fiber )
{
    ...
    CExecThread *curThread = this->GetCurrentThread();

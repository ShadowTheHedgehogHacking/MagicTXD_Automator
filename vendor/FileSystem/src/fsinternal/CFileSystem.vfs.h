...
public:
    ...
    public:
        ...
        template <typename allocatorType>
        inline const fsActiveEntry* FindChildObject( const eir::MultiString <allocatorType>& dirName ) const
        {
        ...
        {
            const fsActiveEntry *foundItem = FindChildObject( dirName );

            if ( foundItem == nullptr || foundItem->isDirectory == false )
            {
                return nullptr;
            }

            return (const directory*)foundItem;
        }

        template <typename allocatorType>
        inline const file*  FindFile( const eir::MultiString <allocatorType>& fileName ) const
        {
            const fsActiveEntry *foundItem = FindChildObject( fileName );

            if ( foundItem == nullptr || foundItem->isFile == false )
            {
                return nullptr;
            }

            return (const file*)foundItem;
        }

        template <typename allocatorType>
        inline fsActiveEntry* FindChildObject( const eir::MultiString <allocatorType>& dirName )
        {
        ...
        {
            fsActiveEntry *foundItem = FindChildObject( dirName );
        ...
        {
            fsActiveEntry *foundItem = FindChildObject( fileName );
...
public:
    ...
    mutable AVLTree <fsNodeSortByOrderDispatcher> fsItemSortedByOrderTree;
    ...
    inline directory* MakeDeriviateDir( directory& root, const normalNodePath& nodePath, bool createMissingDirs, bool hasOrderBy = false, size_t orderBy = 0 )
    {
    ...
    {
        directory *targetDir = MakeDeriviateDir( *m_curDirEntry, dirOfFile, createParentDirs, hasOrdering, ordering );
...
public:
    inline bool RenameObject( internalFilePath newName, const normalNodePath& rootPath, fsActiveEntry *object )
    {
        ...
        directory *targetDir = this->MakeDeriviateDir( this->m_rootDir, rootPath, true );
    ...
    inline bool CreateDir( const char *path )
    {
        ...
        return ( MakeDeriviateDir( *m_curDirEntry, normalPath, true ) != nullptr );
    }
    ...
    inline CFile* OpenStream( const char *path, const filesysOpenMode& mode )
    {
        ...
        bool newFileEntry = false;
        {
            ...
            {
                directory *dir = MakeDeriviateDir( *this->m_curDirEntry, normalPath, mode.createParentDirs );

                if ( dir != nullptr )
                {
                    if ( dir->FindChildObject( name ) == nullptr )
                    {
            ...
            else if ( openType == eFileOpenDisposition::OPEN_OR_CREATE )
            {
                ...
                if ( dir != nullptr )
                {
                    if ( fsActiveEntry *existing = dir->FindChildObject( name ) )
                    {
                        entry = ( existing->isFile ? (file*)existing : nullptr );
...
private:
    ...
    struct vfs_fsitem_iterator
    {
        ...
        inline bool Next( info_data& dataOut )
        {
            ...
            // TOOD: actually implement this key-based safe iterator inside the Eir SDK because
...
public:
        ...
        pathCallback_t dirCallback, pathCallback_t fileCallback, void *userdata ) const
    {
        ...
        decltype(patternEnv)::filePattern_t pattern = patternEnv.CreatePattern( wildcard );

...
long CMemoryMappedFile::Tell( void ) const noexcept
{
    return (long)memStream.Tell();
}

fsOffsetNumber_t CMemoryMappedFile::TellNative( void ) const noexcept
{
    return memStream.Tell();
}

bool CMemoryMappedFile::IsEOF( void ) const noexcept
{
    return ( memStream.Tell() >= memStream.Size() );
}

bool CMemoryMappedFile::QueryStats( filesysStats& statsOut ) const noexcept
{
...
size_t CMemoryMappedFile::GetSize( void ) const noexcept
{
    return (size_t)memStream.Size();
}

fsOffsetNumber_t CMemoryMappedFile::GetSizeNative( void ) const noexcept
{

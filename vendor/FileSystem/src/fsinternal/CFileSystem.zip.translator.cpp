...
void CZIPArchiveTranslator::fileMetaData::ChangeToState( eFileDataState changeToState )
{
    ...
    try
    {
        ...
        else if ( comprType == 8 )
        {
            if ( doCompress )
            {
                compression_progress proc;

                zip_deflate_compression comp( proc, 9, false );
...
long zipStream::Tell( void ) const noexcept
{
    return (long)this->m_ourSeek;
}

fsOffsetNumber_t zipStream::TellNative( void ) const noexcept
{
    return this->m_ourSeek;
}

bool zipStream::IsEOF( void ) const noexcept
{
    return ( this->m_ourSeek >= this->GetSizeNative() );
}

bool zipStream::QueryStats( filesysStats& statsOut ) const noexcept
{
...
size_t zipStream::GetSize( void ) const noexcept
{
    // Just redirect it.
    return (size_t)this->GetSizeNative();
}

fsOffsetNumber_t zipStream::GetSizeNative( void ) const noexcept
{
    CZIPArchiveTranslator::file *fileInfo = &this->m_info;

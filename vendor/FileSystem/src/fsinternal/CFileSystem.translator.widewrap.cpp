...
bool CFileTranslatorWideWrap::CreateDir( const wchar_t *path )
{
    ...
    return CreateDir( widePath.c_str() );
}
...
// UTF-8 support.
bool CFileTranslatorWideWrap::CreateDir( const char8_t *path )
{
    filePath widePath( path );
    widePath.transform_to <char> ();
...
bool CFileTranslatorWideWrap::Delete( const char8_t *path )
{
    filePath widePath( path );
    widePath.transform_to <char> ();

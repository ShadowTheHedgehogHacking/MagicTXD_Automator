...
inline const charType* GetReadWriteMode( bool isNew )
{
    ...
    {
        return ( isNew ? (const char8_t*)"wb" : (const char8_t*)"rb+" );
    }
    else
    {
...
inline const charType* GetOpenArchiveFileMode( bool writeAccess )
{
    ...
    {
        return ( writeAccess ? (const char8_t*)"rb+" : (const char8_t*)"rb" );
    }
    else
    {
        // Only available for MSVC because it is a smarter compiler.
#ifdef _MSC_VER
